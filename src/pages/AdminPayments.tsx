  ...
  return (
    <div className="min-h-screen bg-slate-950 text-white">
      <nav className="sticky top-0 z-50 border-b border-white/10 bg-slate-900/90 backdrop-blur px-6 py-4">
        <div className="mx-auto max-w-6xl flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <span className="rounded-full bg-purple-500/10 border border-purple-400/30 text-purple-300 text-xs px-3 py-1">Admin</span>
          </div>
          <div className="flex items-center gap-2">
            <button onClick={() => navigate("/admin")} className="text-slate-300 hover:text-white text-sm px-3 py-2 rounded-xl hover:bg-white/5 transition">← Dashboard</button>
      ...
      <div className="mx-auto max-w-6xl px-6 py-10">
        <div className="mb-8">
          <p className="text-emerald-400 uppercase tracking-[0.2em] text-xs font-semibold">Admin · LKR</p>
        ...
        {stats && (
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
            {[
              ...
              { label: "Advances Collected", value: lkr(stats.advanceRevenue), icon: "📥", border: "border-cyan-400/20", text: "text-cyan-400" },
              { label: "Balances Collected", value: lkr(stats.balanceRevenue), icon: "✅", border: "border-emerald-400/20", text: "text-emerald-400" },
              ...
              <div key={stat.label} className={`rounded-2xl border ${stat.border} bg-slate-900 p-5`}>
                ...
                <p className={`mt-3 text-xl font-bold ${stat.text}`}>{stat.value}</p>
            ...
            <table className="w-full text-sm min-w-[900px]">
              <thead>
                <tr className="border-b border-white/10 text-slate-400 text-xs uppercase tracking-wider">
                  ...
                  <th className="px-6 py-4 text-left">Amount (LKR)</th>
              ...
              <tbody className="divide-y divide-white/5">
                {payments.map((payment) => (
                  <tr key={payment._id} className="hover:bg-white/5 transition">
                    <td className="px-6 py-4 font-mono text-xs text-cyan-400">{payment.transactionId}</td>
                    <td className="px-6 py-4">
                      <p className="font-medium">{payment.user?.name}</p>
                      <p className="text-slate-400 text-xs">{payment.user?.email}</p>
                    </td>
                    <td className="px-6 py-4">
                      <p className="font-medium">{payment.booking?.tour?.title || "Tour"}</p>
                      <p className="text-slate-400 text-xs">{payment.booking?.tour?.location}</p>
                    </td>
                    <td className="px-6 py-4">
                      <span className={`rounded-full px-3 py-1 text-xs font-semibold border ${
                        ...
                        {payment.paymentType === "advance" ? "Advance (30%)" : "Balance (70%)"}
                      </span>
                    </td>
                    <td className="px-6 py-4 text-slate-400 capitalize">{payment.method.replace("_", " ")}</td>
                    <td className="px-6 py-4 text-emerald-400 font-bold">{lkr(payment.amount)}</td>
                    <td className="px-6 py-4 text-slate-400">{new Date(payment.createdAt).toLocaleDateString()}</td>
                    <td className="px-6 py-4">
                      <select
                        ...
                        className={`rounded-lg border px-3 py-1.5 text-xs outline-none transition disabled:opacity-50 ${
                          payment.status === "completed" ? "bg-emerald-500/10 border-emerald-400/30 text-emerald-300"
                            : payment.status === "refunded" ? "bg-blue-500/10 border-blue-400/30 text-blue-300"
                            : payment.status === "pending" ? "bg-yellow-500/10 border-yellow-400/30 text-yellow-300"
