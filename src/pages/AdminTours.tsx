...
const emptyForm = {
  ...
  image: "", maxGroupSize: "", availableSlots: "", itinerary: "",
...
const AdminTours = () => {
  ...
  const handleSave = async () => {
    if (!form.title || !form.description || !form.price || !form.duration ||
      !form.location || !form.maxGroupSize || !form.availableSlots) {
    ...
    const payload = {
      ...
      maxGroupSize: Number(form.maxGroupSize), availableSlots: Number(form.availableSlots),
  ...
  const handleDelete = async (id: string) => {
    ...
    try { await deleteTour(id); fetchTours() }
    ...
    <div className="min-h-screen bg-slate-950 text-white">
      ...
      <div className="mx-auto max-w-6xl px-4 md:px-6 py-8 md:py-10">
        <div className="flex items-center justify-between mb-6 md:mb-8">
          <div>
            <p className="text-cyan-400 uppercase tracking-[0.2em] text-xs font-semibold">Admin</p>
            <h1 className="mt-1 text-2xl md:text-3xl font-bold">Manage Tours</h1>
          </div>
          <button
            onClick={openCreate}
            className="rounded-xl bg-gradient-to-r from-cyan-500 to-emerald-500 px-4 md:px-5 py-2 md:py-2.5 text-sm font-semibold text-slate-950 hover:opacity-90 transition"
          >
            + Add Tour
          </button>
        </div>
            ...
            <table className="w-full text-sm min-w-[580px]">
              <thead>
                <tr className="border-b border-white/10 text-slate-400 text-xs uppercase tracking-wider">
                  ...
                  <th className="px-4 md:px-6 py-4 text-left">Location</th>
                  <th className="px-4 md:px-6 py-4 text-left">Price (LKR)</th>
                  <th className="px-4 md:px-6 py-4 text-left">Slots</th>
              ...
              <tbody className="divide-y divide-white/5">
                {tours.map((tour) => (
                  <tr key={tour._id} className="hover:bg-white/5 transition">
                    ...
                    <td className="px-4 md:px-6 py-4 text-slate-400 text-sm">{tour.location}</td>
                    <td className="px-4 md:px-6 py-4 text-cyan-400 font-semibold text-sm">
                      LKR {tour.price?.toLocaleString()}
                    </td>
                    <td className="px-4 md:px-6 py-4 text-slate-400 text-sm">{tour.availableSlots}</td>
                    <td className="px-4 md:px-6 py-4">
                      <span
      ...
      {/* Modal — slides from bottom on mobile, centred on desktop */}
      {showModal && (
        <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center bg-black/70 backdrop-blur-sm px-0 sm:px-4">
          <div className="w-full sm:max-w-xl rounded-t-[2rem] sm:rounded-[2rem] border border-white/10 bg-slate-900 p-5 md:p-6 shadow-2xl max-h-[90vh] overflow-y-auto">
            {/* Mobile drag handle */}
            ...
            <div className="space-y-3">
              ...
              <div>
                ...
                <textarea
                  ...
                  onChange={(e) =>
                    setForm((prev) => ({ ...prev, itinerary: e.target.value }))
                  }
                  rows={2}
